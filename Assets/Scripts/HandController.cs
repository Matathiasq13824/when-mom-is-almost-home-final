...
public class HandController : MonoBehaviour {
    ...
    [Header("Size glow Area")]
    public float glow_area = 2;
	...
	void Update () { handle_controller_behavior();
        ...
        {
            // Check the grabbable object close to the player to make them glow
            for (int i = 0; i < anchors_in_the_scene.Length; i++)
            {

                // If close, glow
                if (anchors_in_the_scene[i].is_available() && Vector3.Distance(playerController.transform.position, anchors_in_the_scene[i].transform.position) < glow_area)
                {
                    foreach (var rend in anchors_in_the_scene[i].GetComponents<MeshRenderer>())
                    {
                        rend.materials = new Material[] { rend.materials[0], glow_mat };
                    }

                }
                // To not make change in every object every frame, just check object not close enough to the player to remove the glow, we do not need to
                // change the objects further
                else if (Vector3.Distance(playerController.transform.position, anchors_in_the_scene[i].transform.position) < (glow_area * 1.5))
                {
                    foreach (var rend in anchors_in_the_scene[i].GetComponents<MeshRenderer>())
                    {
	...
	protected void handle_controller_behavior () {
		...
		//Only one hand need to check the glow effect
